...
{   
	...
	$conn=$con;
		if(!empty($club)||!empty($collegeid)||!empty($email)||empty($whatsapp))
		{
			...
			else
			{
				...
				$SELECT="SELECT collegeid From `$club` where collegeid=? Limit 1";
				$INSERT="INSERT Into `$club`(`collegeid`, `whatsapp`, `email`) values (?,?,?)";
				
				$stmt=$conn->prepare($SELECT);
				$stmt->bind_param("s",$collegeid);
				$stmt->execute();
				$stmt->bind_result($collegeid);
				$stmt->store_result();
				$rnum=$stmt->num_rows;
					...
					if($rnum==0)
					{
						...
						$stmt->bind_param("sss",$collegeid,$whatsapp,$email );
						$stmt->execute();
...
<html>
	<head>
		...
		<title>Club-Registar-Page</title>
			...
			<div class="container" style="background: #ccccd285;">
				...
				<div class="row">
					...
					<div  style="display: table-row;">
						...
						<Select style="display: table-cell;" name="clubname"  required="">
						<?php while($row=mysqli_fetch_assoc($q)) 
						{ 
						?>
						 <option value="<?php echo $row['clubname']; ?>"><?php echo $row['clubname']; ?></option>
						<?php
					    }
						?>
					</Select>
