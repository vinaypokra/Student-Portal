...
<html>
	...
	<body>
		<section>
			<center>
				<form style="display: table;padding-top: 20vh;" method="post" >
					<label style="display: table-cell;">Enter College ID : </label>
					<input style="display: table-cell;" type="text" name="Name" placeholder="College ID" required="">
					<input style="display: table-cell;" id="pro"  type="submit"  name="select-info" value="show-Profile">
				</form>
			</center>
		</section>
		<dir>
			<center><p><?php if (isset($_POST['Name'])){ if(strpos($mystring, $word) !== false){ echo "ID check = valid : ";} else{  echo "Invalid ID : ";}}  if($row==0){echo "No record found";}else{ echo "Record found"; }?></p></center>
		</dir>
		<?php
			if($row>0){
		 ?>
